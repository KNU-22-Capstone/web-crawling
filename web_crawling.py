        ...
        for i in tag.keys():
            for j in tag[i]:
                for n in range(1, page_size+1):
                    ...
                    url = f'https://www.musinsa.com/categories/item/{numberring[j]}?d_cat_cd={numberring[j]}&list_kind=small&sort=pop_category&page={n}&display_cnt=90'
                    print(url)
                    ...
                    for ai in range(len(article_info)):
                        ...
                        picture = 'https:' + picture[:-7] + '500' + picture[-4:]    # 이미지 확대
