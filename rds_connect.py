...
def connect_RDS():
    host = "tonton-rds-instance.cf3dpkyx3sfk.ap-northeast-2.rds.amazonaws.com"
    user = "admin"
    password = "tonton1234"
    dbname = "tontnon_rds_name"
    port = 3306
    try:
...
def insert(cursor, dic, count):
    try:
        cursor.execute(
        "INSERT INTO clothes (detail_tag, major_tag, name, picture_url,price, site_name, site_url, sold, views) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);",
        (dic["detail_tag"], dic["major_tag"], dic["name"],dic["picture_URL"],dic["price"],dic["site_name"], dic["site_URL"], False, 0))
